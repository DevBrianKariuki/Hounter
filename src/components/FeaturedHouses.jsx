    ...
    <div className='w-full py-12 flex flex-col'>
        <div className='flex justify-between flex-col md:flex-row  items-center'>
            <div className='flex gap-2 flex-col'>
                ...
                <h1 className='text-3xl font-lexend font-bold text-primary'>Featured Houses</h1>
            </div>
            <div className='flex gap-3'>
                <button className={`${active === 'House' ? 'bg-greenbutton border-none text-green' : 'text-neutral-400 border-neutral-400  text-sm'} py-2 px-4  flex gap-2 border-[1px] rounded-full items-center font-lexend `}  onClick={() =>(setactive('House'))}>
                    <FaHouse />
                    House
                </button>
                <button className={`${active === 'Villa' ? 'bg-greenbutton border-none text-green' : 'text-neutral-400 border-neutral-400  text-sm'} py-2 px-4  flex gap-2 border-[1px] rounded-full items-center font-lexend `}  onClick={() =>(setactive('Villa'))}>
                <MdVilla />
                    Villa
                </button>
                <button className={`${active === 'Apartment' ? 'bg-greenbutton border-none text-green' : 'text-neutral-400 border-neutral-400  text-sm'} py-2 px-4  flex gap-2 border-[1px] rounded-full items-center font-lexend `}  onClick={() =>(setactive('Apartment'))}>
                    <MdApartment />
                    Apartment
                </button>
            </div>
...
const FeatureCard = ({photo, title,icon, price, seller, category,  sellerpic, location}) => (
    <div className='flex w-[100%] hover:scale-105 cursor-pointer flex-col gap-2'>
        ...
        <div className='flex flex-col py-2'>
            <p className='font-lexend hover:text-green font-bold text-primary text-lg'>{title}</p>
            <p className='text-sm font-lexend text-fadeblue'>{price}</p>
        </div>
        <div className='flex items-center  gap-2'>
            <img src={sellerpic} className='rounded-full w-[40px] h-[40px]  object-cover' />
