      ...
      <div className="flex flex-col px-6 md:flex-row w-full my-5">
        <div className="w-full flex flex-col gap-4 justify-center">
          {blogs.map((eachblog) => (
            <BlogCard
              key={eachblog.id}
              picture={eachblog.picture}
              writer_pic={eachblog.writer_pic}
              writer={eachblog.writer}
...
const BlogCard = ({ picture, writer_pic, writer, title, time }) => (
  <div className="w-full hover:cursor-pointer hover:scale-105 flex py-2  gap-5">
    ...
    <div className="flex flex-col justify-center gap-1">
      ...
      <h1 className="font-lexend font-medium  md:w-[80%]  hover:font-semibold text-primary">
        {title}
      </h1>
